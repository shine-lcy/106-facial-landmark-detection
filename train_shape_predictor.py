...
#     exit()
faces_folder = '../dlib-19.6/examples/faces/size'
...
# parts.
training_xml_path = os.path.join(faces_folder, "all_max_2.xml")
...
# in the Kazemi paper.
# testing_xml_path = os.path.join(faces_folder, "3901_max_2.xml")
...
predictor_68 = dlib.shape_predictor("./dlib-19.6/python_examples/shape_predictor_68_face_landmarks.dat")
...
def get_detection(img_path):
    ...
    if len(dets) == 0:
        flag = 0
        return x_list, y_list, flag
    shape = predictor_68(img, dets[0])

    for i in range(68):
   		x_list.append(shape.part(i).x)
		y_list.append(shape.part(i).y)
    return x_list, y_list ,flag
#x_list = get_detection("/home/lc/106_resnet/3.jpg")
#print x_list
# print("Showing detections and predictions on the images in the faces folder...")
# win = dlib.image_window()
# for f in glob.glob(os.path.join(faces_folder, "*.jpg")):
...
#             cv2.circle(img, (shape.part(i).x,shape.part(i).y), 2, (55, 255, 155), 1)
#         cv2.imshow("Image", img)
#         cv2.waitKey(0)
'''
f = "/home/lc/106_resnet/show/3.jpg"
print("Processing file: {}".format(f))
x_list, y_list = get_detection(f)
print len(x_list)
...
for k, d in enumerate(dets):
    ...
    for i in range(106):
        ...
        cv2.circle(img, (shape.part(i).x,shape.part(i).y), 2, (55, 255, 155), 1)
    cv2.imshow("Image", img)
    cv2.imwrite("/home/lc/aaa.jpeg",img)
    cv2.waitKey(0)
'''
